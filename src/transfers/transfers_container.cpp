...
namespace cryptonote
{
    ...
    SpentOutputDescriptor::SpentOutputDescriptor(const TransactionOutputInformationIn &transactionInfo) : m_type(transactionInfo.type),
                                                                                                          m_amount(0),
                                            ...
                                            const std::vector<TransactionOutputInformationIn> &transfers)
    {
        ...
        catch (...)
        {
            if (m_transactions.count(tx.getTransactionHash()) == 0)
            {
                m_logger(ERROR, BRIGHT_RED) << "Failed to add transaction, remove transaction transfers, block " << block.height << ", transaction hash " << tx.getTransactionHash();
            ...
            else
            {
                if (info.type == TransactionTypes::OutputType::Key)
                {
                    ...
                    if (duplicate)
                    {
                        ...
                        m_logger(ERROR, BRIGHT_RED) << message << ", transaction hash " << info.transactionHash << ", output index " << info.outputInTransaction << ", key image " << info.keyImage;
            ...
            if (inputType == TransactionTypes::InputType::Key)
            {
                ...
                if (std::distance(spentRange.first, spentRange.second) > 0)
                {
                    ...
                    m_logger(ERROR, BRIGHT_RED) << message << ", key image " << input.keyImage << '\n'
                                                ...
                                                << ": hash " << tx.getTransactionHash() << ", block " << block.height << ", transaction index " << block.transactionIndex << ", input " << i << '\n'
                                                << "    spending transaction"
                                                << ": hash " << spentOutput.spendingTransactionHash << ", block " << spentOutput.spendingBlock.height << ", input " << spentOutput.inputInTransaction << '\n'
                                                << "    spent output        "
                                                << ": hash " << spentOutput.transactionHash << ", block " << spentOutput.blockHeight << ", transaction index " << spentOutput.transactionIndex << ", output " << spentOutput.outputInTransaction << ", amount " << m_currency.formatAmount(spentOutput.amount);
                    throw std::runtime_error(message);
                }
                                                      ...
                                                      const std::vector<uint32_t> &globalIndices)
    {
        ...
        {
            m_logger(ERROR, BRIGHT_RED) << "markTransactionConfirmed failed: " << e.what() << ", rollback changes, block index " << block.height << ", tx " << transactionHash;
