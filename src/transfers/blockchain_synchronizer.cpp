...
namespace cryptonote
{
    ...
    std::error_code BlockchainSynchronizer::doAddUnconfirmedTransaction(const ITransactionReader &transaction)
    {
        ...
        for (; addIt != m_consumers.end(); ++addIt)
        {
            ...
            if (ec)
            {
                m_logger(ERROR, BRIGHT_RED) << "Failed to add unconfirmed transaction to consumer: " << ec << ", " << ec.message() << ", consumer " << addIt->first << ", hash " << transaction.getTransactionHash();
                break;
            }
        }
    ...
    void BlockchainSynchronizer::startPoolSync()
    {
        ...
        { // get union ok
            ...
            {
                m_logger(DEBUGGING) << "Transaction pool changes received, added " << unionResponse.newTxs.size() << ", deleted " << unionResponse.deletedTxIds.size();
                ...
                else
                {
                    ...
                    { // get intersection ok
                        ...
                        {
                            m_logger(DEBUGGING) << "Transaction pool changes at stage 2 received, added " << intersectionResponse.newTxs.size() << ", deleted " << intersectionResponse.deletedTxIds.size();
