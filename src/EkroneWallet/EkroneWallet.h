...
namespace cn
{
  ...
  /*                                                                      */
  /************************************************************************/
  class ekrone_wallet : public cn::INodeObserver, public cn::IWalletLegacyObserver, public cn::INodeRpcProxyObserver {
  public:
    ekrone_wallet(platform_system::Dispatcher& dispatcher, const cn::Currency& currency, logging::LoggerManager& log);
  ...
  private:
    ...
    {
    public:
      refresh_progress_reporter_t(cn::ekrone_wallet& ekrone_wallet)
        : m_ekrone_wallet(ekrone_wallet)
      ...
      void update(uint64_t height, bool force = false)
      {
        ...
        if (std::chrono::seconds(m_ekrone_wallet.currency().difficultyTarget() / 2) < current_time - m_blockchain_height_update_time ||
    ...
    private:
      ...
      {
        uint64_t blockchain_height = m_ekrone_wallet.m_node->getLastLocalBlockHeight();
        m_blockchain_height = blockchain_height;
        m_blockchain_height_update_time = std::chrono::system_clock::now();
      }

    private:
      cn::ekrone_wallet& m_ekrone_wallet;
