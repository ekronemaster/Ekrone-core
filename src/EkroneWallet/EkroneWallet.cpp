...
bool processServerAliasResponse(const std::string& s, std::string& address) {
  ...
  // Courtesy of Monero Project
		// make sure the txt record has "oa1:xekr" and find it
		auto pos = s.find("oa1:xekr");
...
ekrone_wallet::ekrone_wallet(platform_system::Dispatcher& dispatcher, const cn::Currency& currency, logging::LoggerManager& log) :
  ...
  m_consoleHandler.setHandler("create_integrated", boost::bind(&ekrone_wallet::create_integrated, this, boost::arg<1>()), "create_integrated <payment_id> - Create an integrated address with a payment ID");
  m_consoleHandler.setHandler("export_keys", boost::bind(&ekrone_wallet::export_keys, this, boost::arg<1>()), "Show the secret keys of the current wallet");
  m_consoleHandler.setHandler("balance", boost::bind(&ekrone_wallet::show_balance, this, boost::arg<1>()), "Show current wallet balance");
  m_consoleHandler.setHandler("sign_message", boost::bind(&ekrone_wallet::sign_message, this, boost::arg<1>()), "Sign a message with your wallet keys");
  m_consoleHandler.setHandler("verify_signature", boost::bind(&ekrone_wallet::verify_signature, this, boost::arg<1>()), "Verify a signed message");
  m_consoleHandler.setHandler("incoming_transfers", boost::bind(&ekrone_wallet::show_incoming_transfers, this, boost::arg<1>()), "Show incoming transfers");
  m_consoleHandler.setHandler("list_transfers", boost::bind(&ekrone_wallet::listTransfers, this, boost::arg<1>()), "list_transfers <height> - Show all known transfers from a certain (optional) block height");
  m_consoleHandler.setHandler("payments", boost::bind(&ekrone_wallet::show_payments, this, boost::arg<1>()), "payments <payment_id_1> [<payment_id_2> ... <payment_id_N>] - Show payments <payment_id_1>, ... <payment_id_N>");
  m_consoleHandler.setHandler("get_tx_proof", boost::bind(&ekrone_wallet::get_tx_proof, this, boost::arg<1>()), "Generate a signature to prove payment: <txid> <address> [<txkey>]");
  m_consoleHandler.setHandler("bc_height", boost::bind(&ekrone_wallet::show_blockchain_height, this, boost::arg<1>()), "Show blockchain height");
  m_consoleHandler.setHandler("show_dust", boost::bind(&ekrone_wallet::show_dust, this, boost::arg<1>()), "Show the number of unmixable dust outputs");
  m_consoleHandler.setHandler("outputs", boost::bind(&ekrone_wallet::show_num_unlocked_outputs, this, boost::arg<1>()), "Show the number of unlocked outputs available for a transaction");
  m_consoleHandler.setHandler("optimize", boost::bind(&ekrone_wallet::optimize_outputs, this, boost::arg<1>()), "Combine many available outputs into a few by sending a transaction to self");
  m_consoleHandler.setHandler("optimize_all", boost::bind(&ekrone_wallet::optimize_all_outputs, this, boost::arg<1>()), "Optimize your wallet several times so you can send large transactions");  
  m_consoleHandler.setHandler("transfer", boost::bind(&ekrone_wallet::transfer, this, boost::arg<1>()),
    "transfer <addr_1> <amount_1> [<addr_2> <amount_2> ... <addr_N> <amount_N>] [-p payment_id]"
    " - Transfer <amount_1>,... <amount_N> to <address_1>,... <address_N>, respectively. ");
  m_consoleHandler.setHandler("set_log", boost::bind(&ekrone_wallet::set_log, this, boost::arg<1>()), "set_log <level> - Change current log level, <level> is a number 0-4");
  m_consoleHandler.setHandler("address", boost::bind(&ekrone_wallet::print_address, this, boost::arg<1>()), "Show current wallet public address");
  m_consoleHandler.setHandler("save", boost::bind(&ekrone_wallet::save, this, boost::arg<1>()), "Save wallet synchronized data");
  m_consoleHandler.setHandler("reset", boost::bind(&ekrone_wallet::reset, this, boost::arg<1>()), "Discard cache data and start synchronizing from the start");
  m_consoleHandler.setHandler("help", boost::bind(&ekrone_wallet::help, this, boost::arg<1>()), "Show this help");
  m_consoleHandler.setHandler("ext_help", boost::bind(&ekrone_wallet::extended_help, this, boost::arg<1>()), "Show this help");
  m_consoleHandler.setHandler("exit", boost::bind(&ekrone_wallet::exit, this, boost::arg<1>()), "Close wallet");  
  m_consoleHandler.setHandler("balance_proof", boost::bind(&ekrone_wallet::get_reserve_proof, this, boost::arg<1>()), "all|<amount> [<message>] - Generate a signature proving that you own at least <amount>, optionally with a challenge string <message>. ");
  ...
  m_consoleHandler.setHandler("save_txs_to_file", boost::bind(&ekrone_wallet::save_all_txs_to_file, this, boost::arg<1>()), "save_txs_to_file - Saves all known transactions to <wallet_name>_ekrone_transactions.txt");
  m_consoleHandler.setHandler("check_address", boost::bind(&ekrone_wallet::check_address, this, boost::arg<1>()), "check_address <address> - Checks to see if given wallet is valid.");
}
...
std::string ekrone_wallet::wallet_menu(bool do_ext)
{
  ...
  if (do_ext)
  {
    menu_item += "\t\tEkrone Wallet Extended Menu\n\n";
...
bool ekrone_wallet::new_wallet(const std::string &wallet_file, const std::string& password) {
  ...
  success_msg_writer() <<
    ...
    "Use \"help\" command to see the list of available commands.\n" <<
    "Always use \"exit\" command when closing Ekrone Wallet to save\n" <<
...
bool ekrone_wallet::get_reserve_proof(const std::vector<std::string> &args)
{
	if (args.size() != 1 && args.size() != 2)
  {
	...
	try {
		...
		const std::string filename = "balance_proof_" + args[0] + "_EKR.txt";
...
bool ekrone_wallet::save_keys_to_file(const std::vector<std::string>& args)
{
  ...
  std::string formatted_wal_str = m_frmt_wallet_file + "_ekrone_backup.txt";
...
bool ekrone_wallet::save_all_txs_to_file(const std::vector<std::string> &args)
{
  ...
  std::string formatted_wal_str = m_frmt_wallet_file + "_ekrone_transactions.txt";
  ...
  logger(INFO, BRIGHT_GREEN) << "All transactions have been saved to the current folder where \"ekronewallet\" is located as \""
    << formatted_wal_str << "\".";
  ...
  if (include_deposits == true)
  {
    ...
    /* tell user job complete */
    logger(INFO, BRIGHT_GREEN) << "All deposits have been saved to the end of the file current folder where \"ekronewallet\" is located as \""
      << formatted_wal_str << "\".";
  }
...
bool ekrone_wallet::check_address(const std::vector<std::string> &args)
{
  if (args.size() != 1) 
  {
