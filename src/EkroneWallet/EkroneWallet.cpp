...
bool processServerAliasResponse(const std::string& s, std::string& address) {
  ...
  // Courtesy of Monero Project
		// make sure the txt record has "oa1:ekrone" and find it
		auto pos = s.find("oa1:ekrone");
		if (pos == std::string::npos)
			return false;
...
std::string ultranote_wallet::wallet_menu(bool do_ext)
{
  ...
  if (do_ext)
  {
    menu_item += "\t\tEkrone Gold Wallet Extended Menu\n\n";
  ...
  {
    menu_item += "\t\tEkrone Gold Wallet Menu\n\n";
    menu_item += "[ ] = Optional arg\n\n";
...
bool ultranote_wallet::get_reserve_proof(const std::vector<std::string> &args)
{
	...
	try {
		...
		const std::string filename = "reserve_proof_" + args[0] + "_ekrone.txt";
