...
namespace cryptonote
{
    ...
    {
    public:
        ScopeExitHandler(std::function<void()> &&handler) : m_handler(std::move(handler)),
                                                            m_cancelled(false)
        {
        }
    ...
    void BlockchainExplorer::blockchainSynchronized(uint32_t topIndex)
    {
        ...
        request.performAsync(asyncContextCounter,
                             [this, blockIndexesPtr, blocksPtr](std::error_code ec)
                             {
                                 ...
                                 auto mainchainBlockIter = std::find_if_not(blocksPtr->front().cbegin(), blocksPtr->front().cend(), [](const BlockDetails &block)
