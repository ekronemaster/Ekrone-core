...
namespace
{
  const command_line::arg_descriptor<std::string> arg_config_file = {"config-file", "Specify configuration file", "ekrone.conf"};
...
void renameDataDir() {
  std::string ekroneIDir = tools::getDefaultDataDirectory();
  boost::filesystem::path ekroneIDirPath(ekroneIDir);
  if (boost::filesystem::exists(ekroneIDirPath)) {
    return;
  }

  std::string dataDirPrefix = ekroneIDir.substr(0, ekroneIDir.size() + 1 - sizeof(CRYPTONOTE_NAME));
  boost::filesystem::path cediDirPath(dataDirPrefix + "BXC");

  if (boost::filesystem::exists(cediDirPath)) {
    boost::filesystem::rename(cediDirPath, ekroneIDirPath);
  } else {
    boost::filesystem::path BcediDirPath(dataDirPrefix + "Bcedi");
    if (boost::filesystem::exists(boost::filesystem::path(BcediDirPath))) {
		boost::filesystem::rename(BcediDirPath, ekroneIDirPath);
    }
  }
}
