...
namespace
{
  ...
  {
    uint64_t val = base_58::uint_8be_to_64(data, size);
  ...
  void do_test_uint_64_to_8be(uint64_t num, const std::string& expected_str)
  {
    ...
    base_58::uint_64_to_8be(num, data.size(), reinterpret_cast<uint8_t*>(&data[0]));
    ASSERT_EQ(data, expected_str);
  }

  void do_test_encode_block(const std::string& block, const std::string& expected)
  {
    ASSERT_TRUE(1 <= block.size() && block.size() <= base_58::full_block_size);
    std::string enc(base_58::encoded_block_sizes[block.size()], base_58::alphabet[0]);
    base_58::encode_block(block.data(), block.size(), &enc[0]);
    ASSERT_EQ(enc, expected);

    std::string dec(block.size(), '\0');
    ASSERT_TRUE(base_58::decode_block(enc.data(), enc.size(), &dec[0]));
    ASSERT_EQ(block, dec);
  }

  void do_test_decode_block_pos(const std::string& enc, const std::string& expected)
  {
    std::string data(base_58::decoded_block_sizes::instance(enc.size()), '\0');
    ASSERT_TRUE(base_58::decode_block(enc.data(), enc.size(), &data[0]));
    ASSERT_EQ(data, expected);
  }

  void do_test_decode_block_neg(const std::string& enc)
  {
    std::string data(base_58::full_block_size, '\0');
    ASSERT_FALSE(base_58::decode_block(enc.data(), enc.size(), &data[0]));
  }

  void do_test_encode(const std::string& data, const std::string& expected)
  {
    std::string enc = base_58::encode(data);
    ASSERT_EQ(enc, expected);

    std::string dec;
    ASSERT_TRUE(base_58::decode(enc, dec));
    ASSERT_EQ(dec, data);
  }
  ...
  {
    std::string dec;
    ASSERT_FALSE(base_58::decode(enc, dec));
  }

  void do_test_encode_decode_addr(uint64_t tag, const std::string& data, const std::string& expected)
  {
    std::string addr = base_58::encode_addr(tag, data);
    ASSERT_EQ(addr, expected);

    uint64_t dec_tag;
    std::string dec_data;
    ASSERT_TRUE(base_58::decode_addr(addr, dec_tag, dec_data));
...
{
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance(0));
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance(std::numeric_limits<char>::min()));
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance(std::numeric_limits<char>::max()));
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance('1' - 1));
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance('z' + 1));
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance('0'));
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance('I'));
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance('O'));
  ASSERT_EQ(-1, base_58::reverse_alphabet::instance('l'));
  ASSERT_EQ(0,  base_58::reverse_alphabet::instance('1'));
  ASSERT_EQ(8,  base_58::reverse_alphabet::instance('9'));
  ASSERT_EQ(base_58::alphabet_size - 1, base_58::reverse_alphabet::instance('z'));
}
...
#define TEST_decode_addr_neg(addr, test_name)                     \
  ...
  {                                                               \
    uint64_t tag;                                                 \
    std::string data;                                             \
    ASSERT_FALSE(base_58::decode_addr(MAKE_STR(addr), tag, data)); \
  }
...
{
  std::string addr_str = base_58::encode_addr(0, test_serialized_keys);
...
{
  std::string addr_str = base_58::encode_addr(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, test_serialized_keys.substr(1));
...
TEST(parseAccountAddressString, fails_on_invalid_address_spend_key)
{
  ...
  std::string addr_str = base_58::encode_addr(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, serialized_keys_copy);
