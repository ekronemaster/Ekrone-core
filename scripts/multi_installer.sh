...
# Ekrone Multi-installer
# a one line clone-and-compile for ekrone:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/ekrone/ekrone/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/ekrone ]; then
            read -r -p "${1:-ekrone directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old ekrone directory" && echo
                    rm -rf "$PWD"/ekrone
                    ;;
                *)
                    _fail "ekrone directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir ekrone
...
_build_ekrone() {
    _note "Building ekrone from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/ekrone')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/ekrone')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Ekrone Multi_Installer v1.0 (pepperoni)"
